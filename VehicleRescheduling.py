        ...
        if positions.count(train.position) > 1:
            if train.position not in self.stations.values() and train.position not in self.railway_switch.values():
                for v in self.trains:
                    if v.position == train.position and (v.position not in self.stations.values() and train.position not in self.railway_switch.values()):
...
def evaluate_agent(env, agent, n_eval_episodes=1000, max_steps=200, q_table = None, viz_on = 1):
    rewards_per_episode = []
    deltas_arrival = []
...
agents = [QLearningAgent(env.total_states, 3, learning_rate=0.2, discount_factor=0.8, exploration_rate=0.8, exploration_decay=0.995) for _ in env.trains]
...
agents_delayed = [QLearningAgent(env_delayed.total_states, 3, learning_rate=0.2, discount_factor=0.8, exploration_rate=0.5, exploration_decay=0.95) for _ in env_delayed.trains]
